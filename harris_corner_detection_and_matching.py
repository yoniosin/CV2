...
def main():
    ...
    image = [cv2.imread(im) for im in frames_names]
    # corner = [cornerDetector(img) for img in frames]
    ...
    for i in range(len(image)):
        ...
        for j in range(desired_features.shape[0]):
            cv2.circle(harris_corner_desired, ((i * width) + desired_features.item(j, 1), desired_features.item(j, 0)),
                       0, (255, 0, 0), 4)
